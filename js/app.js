...
function renderAdminProducts() {
  ...
  const rows = state.adminProducts.map((p) => {
    ...
    return `<tr>
      ...
      <td class="flex gap-2">
        <button class="editProductBtn text-gold/70 hover:text-gold text-xs" data-id="${p.id}">Editar</button>
  ...
  wrap.innerHTML = `<table class="data">
    <thead><tr><th>Producto</th><th>Categoría</th><th>Precio</th><th>Margen</th><th>Stock</th><th>Estado</th><th>Acciones</th></tr></thead>
    <tbody>${rows || `<tr><td colspan="7" class="text-cream/40 text-center py-6">Aún no hay productos</td></tr>`}</tbody>
  </table>`;

  wrap.querySelectorAll(".editProductBtn").forEach((b) =>
    b.addEventListener("click", () => openProductModal(state.adminProducts.find((p) => p.id === b.dataset.id))));
...
function openProductModal(product) {
  ...
  document.querySelector("#modalProduct h3").textContent = product ? "Editar producto" : "Nuevo producto";

  state.editingProductId = product ? product.id : null;
  const tracksStock = product && product.currentStock !== undefined;
  document.getElementById("pStock").classList.toggle("hidden", !tracksStock);
  document.getElementById("pMinStock").classList.toggle("hidden", !tracksStock);
  document.getElementById("pTracksStock").checked = tracksStock;

  if (product) {
    document.getElementById("pName").value = product.name || "";
    sel.value = product.categoryId || "";
    document.getElementById("pPrice").value = product.price || "";
    document.getElementById("pCost").value = product.cost || "";
    if (tracksStock) {
      document.getElementById("pStock").value = product.currentStock;
      document.getElementById("pMinStock").value = product.minStockAlert || 0;
    }
  }
...
document.getElementById("productForm").addEventListener("submit", async (e) => {
  ...
  if (tracksStock) {
    ...
    data.currentStock = deleteField();
    data.minStockAlert = deleteField();
  }

  if (state.editingProductId) {
    await updateDoc(docRef("products", state.editingProductId), data);
