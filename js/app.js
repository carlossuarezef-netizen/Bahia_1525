...
/* ============================================================
   ...
   Antes había que recargar la página a mano para ver cambios
   hechos desde otro dispositivo (otra mesa, otro cajero, etc.),
   y eso cerraba la sesión porque el login vive solo en memoria.
   Ahora, cada 20s, refrescamos en silencio SOLO la pantalla que
   está visible en este momento — sin recargar la página y sin
   tocar la sesión — para que los cambios se vean solos.
   ============================================================ */
let autoRefreshTimer = null;

function startAutoRefresh() {
  stopAutoRefresh();
  autoRefreshTimer = setInterval(silentRefreshCurrentScreen, 20000);
}
...
// El refresco automático nunca debe interrumpir a alguien que está
// a mitad de escribir algo o de completar una acción: si hay un
// campo con foco, un modal abierto, o una fila de producto en
// edición, simplemente nos saltamos ese ciclo y probamos en 20s.
...
async function silentRefreshCurrentScreen() {
  ...
  try {
    switch (visible.id) {
      case "screen-tables":
        ...
        break;
      case "screen-pos":
        await refreshPosScreenSilently();
        break;
      case "screen-shift": {
        // Si ya está contando el efectivo real para cerrar, no le
        // pisamos ese campo con un refresco.
        const actualCashInput = document.getElementById("actualCash");
        if (actualCashInput && actualCashInput.value) return;
        await renderShiftScreen();
        break;
      }
      case "screen-expenses":
        await loadExpenseCategories();
        break;
      case "screen-breakfast":
        await renderBreakfastScreen();
        break;
      case "screen-admin":
    ...
    // Un refresco silencioso fallido no debe interrumpir al usuario.
    console.warn("Auto-refresco falló:", err);
  }
}

// Para la pantalla de toma de pedidos: trae el pedido y el stock de
// productos actualizados, sin perder la categoría o búsqueda activa.
async function refreshPosScreenSilently() {
  if (!state.currentOrder || !state.currentTable) return;
  const snap = await getDoc(docRef("orders", state.currentOrder.id));
  if (snap.exists()) {
    state.currentOrder = { id: snap.id, ...snap.data() };
    renderCart();
  }
...
async function openTable(table) {
  ...
  // El mesero solo registra pedidos; cobrar y facturar queda para
  // cajero y administrador.
  const canCharge = ["cajero", "admin_contador"].includes(state.user.role);
  document.getElementById("chargeBtn").classList.toggle("hidden", !canCharge);
...
document.getElementById("chargeBtn").addEventListener("click", () => {
  if (!["cajero", "admin_contador"].includes(state.user.role)) return; // solo cajero/admin facturan
  if (!state.currentOrder.items.length) return toast("El pedido está vacío", "error");
...
function renderCheckout() {
  ...
  document.getElementById("coTipManual").value = checkoutState.tip || "";
...
document.getElementById("coTipManual").addEventListener("input", (e) => {
  checkoutState.tip = parseFloat(e.target.value) || 0;
  const subtotal = cartSubtotal();
  document.getElementById("coTotal").textContent = cop(subtotal + checkoutState.tip);
  renderRemaining();
});
...
document.getElementById("previewReceiptBtn").addEventListener("click", () => {
  const subtotal = cartSubtotal();
  const total = subtotal + checkoutState.tip;
  const clientData = document.getElementById("needsInvoice").checked
    ? {
        name: document.getElementById("invName").value || "Cliente",
...
async function confirmSale() {
  ...
  try {
    const saleRef = await runTransaction(db, async (tx) => {
      ...
      // Firestore exige que, dentro de una transacción, todas las
      // lecturas ocurran antes que cualquier escritura. Con un solo
      // producto en el carrito nunca se notaba; con varios, mezclar
      // lectura+escritura por cada ítem rompía la transacción entera.
      const productReads = [];
      for (const item of state.currentOrder.items) {
        const prodRef = docRef("products", item.productId);
        const prodSnap = await tx.get(prodRef);
        productReads.push({ item, prodRef, prodSnap });
      }

      const ingredientReads = [];
      for (const { item, prodSnap } of productReads) {
        ...
        if (product.isRecipe && product.recipeId) {
          ...
          if (recipeSnap.exists()) {
            for (const ing of recipeSnap.data().items) {
              ...
              ingredientReads.push({ item, ing, ingRef, ingSnap });
            }
          }
        }
      }
      ...
      for (const { item, prodRef, prodSnap } of productReads) {
        if (!prodSnap.exists()) continue;
        const product = prodSnap.data();
        if (!(product.isRecipe && product.recipeId) && product.currentStock !== undefined) {
          tx.update(prodRef, { currentStock: product.currentStock - item.qty });
        }
      }

      // Si dos productos distintos comparten un mismo ingrediente
      // (ej. limón en dos cócteles), Firestore no permite escribir el
      // mismo documento dos veces en una transacción — se agrupa el
      // consumo total por ingrediente y se escribe una sola vez.
      const ingredientConsumption = {};
      for (const { item, ing, ingRef, ingSnap } of ingredientReads) {
        if (!ingSnap.exists()) continue;
        if (!ingredientConsumption[ing.ingredientId]) {
          ingredientConsumption[ing.ingredientId] = { ref: ingRef, currentStock: ingSnap.data().currentStock || 0, subtract: 0 };
        }
        ingredientConsumption[ing.ingredientId].subtract += ing.quantity * item.qty;
      }
      Object.values(ingredientConsumption).forEach(({ ref, currentStock, subtract }) => {
        tx.update(ref, { currentStock: currentStock - subtract });
      });
    ...
    console.error(err);
    toast(`Error al procesar la venta: ${err.message || err}`, "error");
  }
}
...
function showReceipt(sale) {
  ...
  document.getElementById("receiptContent").innerHTML = `
    ${sale.isPreview ? `<div class="text-center mb-2"><span class="chip active">BORRADOR · aún no facturado</span></div>` : ""}
...
/* ============================================================
   PIN (administrador / propio / de cualquier usuario)
   El mismo modal se reutiliza para las tres cosas; solo cambia el
   texto que se muestra y contra qué se valida el PIN ingresado.
   ============================================================ */
function openPinModal(label) {
  document.getElementById("adminPinLabel").textContent = label;
...
// Requiere específicamente el PIN de un usuario con rol administrador
// (anulaciones, quitar un ítem del pedido, etc.).
function requireAdminPin(onSuccess) {
  openPinModal("PIN de administrador requerido");
...
function requireAnyPin(onSuccess) {
  openPinModal("Ingresa tu PIN para continuar");
  document.getElementById("confirmAdminPin").onclick = async () => {
    const pin = document.getElementById("adminPinInput").value;
    const snap = await getDocs(query(col("users"), where("pin", "==", pin), where("active", "==", true)));
    if (snap.empty) return pinError();
...
// Requiere específicamente el PIN del usuario que tiene la sesión
// iniciada en este momento — por ejemplo, el cajero que va a cerrar
// SU PROPIA caja. No sirve el PIN de un compañero ni el del admin.
function requireOwnPin(onSuccess) {
  openPinModal(`Confirma tu PIN, ${state.user.name}`);
  document.getElementById("confirmAdminPin").onclick = () => {
    const pin = document.getElementById("adminPinInput").value;
    if (pin !== state.user.pin) return pinError();
    closeModal("modalAdminPin");
    onSuccess();
  };
}
...
// El input sigue aceptando el teclado físico normalmente (solo dígitos,
// máximo 4, y Enter valida) — el teclado en pantalla es un atajo extra,
// no un reemplazo.
(() => {
  const input = document.getElementById("adminPinInput");
  ...
  input.addEventListener("keydown", (e) => {
    if (e.key === "Enter") { e.preventDefault(); document.getElementById("confirmAdminPin").click(); }
  });
})();

// Teclado numérico en pantalla del modal de PIN (se construye una sola
// vez y se reutiliza para requireAdminPin / requireAnyPin / requireOwnPin).
function renderAdminPinKeypad() {
  const grid = document.getElementById("adminPinKeypad");
  const keys = ["1","2","3","4","5","6","7","8","9","borrar","0","ok"];
  keys.forEach((k) => {
    const btn = document.createElement("button");
    btn.type = "button";
    ...
    btn.addEventListener("click", () => {
      const input = document.getElementById("adminPinInput");
      if (k === "borrar") input.value = input.value.slice(0, -1);
      else if (k === "ok") document.getElementById("confirmAdminPin").click();
...
async function renderDailyCashSummary() {
  ...
  const todayFilter = where("createdAt", ">=", Timestamp.fromDate(startOfToday));

  const [salesSnap, expSnap] = await Promise.all([
    getDocs(query(col("sales"), todayFilter)),
    getDocs(query(col("expenses"), todayFilter)),
  ]);
  ...
  // Igual que en el cierre de turno: solo los egresos pagados EN EFECTIVO
  // salen físicamente de la caja, así que solo esos se restan del efectivo.
  let expensesCash = 0, expensesCount = 0;
  expSnap.forEach((d) => {
    const e = d.data();
    expensesCount += 1;
    if (e.method === "efectivo") expensesCash += e.amount || 0;
  });

  const netCash = cash - expensesCash;
  const total = netCash + card + transfer;

  wrap.innerHTML = `
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
      <div><p class="text-cream/50">Efectivo neto hoy</p><p class="text-lg text-goldsoft">${cop(netCash)}</p></div>
      ...
      <div><p class="text-cream/50">Total neto del día</p><p class="text-lg text-goldsoft">${cop(total)}</p></div>
    </div>
    <p class="text-cream/40 text-xs mt-3">
      ${count} venta(s) hoy (efectivo bruto: ${cop(cash)}) · ${expensesCount} egreso(s) hoy en efectivo: -${cop(expensesCash)} · Propinas: ${cop(tips)}
    </p>
...
/* ---- Caja Mayor: acumulado de TODOS los cierres, por día y total histórico ----
   "Lo que queda" de cada cierre = el sobrante retirado (efectivo por encima
   de la base fija) + todo lo cobrado por tarjeta y transferencia, que nunca
   se queda físicamente en la caja del local. */
async function renderCajaMayor() {
  const wrap = document.getElementById("cajaMayorTable");
  if (!wrap) return;
  wrap.innerHTML = `<p class="text-cream/40 text-sm p-4">Calculando...</p>`;

  const snap = await getDocs(query(col("shifts"), where("status", "==", "cerrado"), orderBy("closedAt", "desc"), limit(200)));
  const byDay = {};
  snap.forEach((d) => {
    const s = d.data();
    const date = s.closedAt?.toDate ? s.closedAt.toDate().toLocaleDateString("es-CO") : "—";
    if (!byDay[date]) byDay[date] = { card: 0, transfer: 0, cashOut: 0 };
    const t = s.totals || {};
    byDay[date].card += t.card || 0;
    byDay[date].transfer += t.transfer || 0;
    // sobranteRetirado ya representa el efectivo que salió de caja por
    // encima de la base fija; si un cierre viejo no lo tiene calculado
    // (versiones anteriores del sistema), se usa la diferencia como respaldo.
    byDay[date].cashOut += t.sobranteRetirado ?? t.difference ?? 0;
  });

  const days = Object.keys(byDay); // ya viene ordenado desde la consulta (más reciente primero)
  let grand = { card: 0, transfer: 0, cashOut: 0 };
  const rows = days.map((date) => {
    const d = byDay[date];
    grand.card += d.card; grand.transfer += d.transfer; grand.cashOut += d.cashOut;
    const total = d.card + d.transfer + d.cashOut;
    return `<tr>
      <td>${date}</td><td>${cop(d.cashOut)}</td><td>${cop(d.card)}</td><td>${cop(d.transfer)}</td>
      <td class="text-goldsoft font-semibold">${cop(total)}</td>
    </tr>`;
  }).join("");
  const grandTotal = grand.card + grand.transfer + grand.cashOut;

  wrap.innerHTML = `<table class="data">
    <thead><tr><th>Día</th><th>Efectivo retirado</th><th>Tarjeta</th><th>Transferencia</th><th>Total del día</th></tr></thead>
    <tbody>${rows || `<tr><td colspan="5" class="text-cream/40 text-center py-6">Aún no hay cierres de caja</td></tr>`}</tbody>
    <tfoot><tr class="bg-gold/10 font-semibold text-goldsoft">
      <td>ACUMULADO TOTAL</td><td>${cop(grand.cashOut)}</td><td>${cop(grand.card)}</td><td>${cop(grand.transfer)}</td><td>${cop(grandTotal)}</td>
    </tr></tfoot>
  </table>`;
}
