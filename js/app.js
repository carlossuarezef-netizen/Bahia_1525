...
function renderAdminCategories() {
  ...
  if (!wrap) return;
  wrap.innerHTML = state.categories.map((c) => `<span class="chip active">${c.name}</span>`).join("")
    || `<span class="text-cream/40 text-sm">Aún no hay categorías</span>`;
}
...
document.getElementById("categoryForm")?.addEventListener("submit", async (e) => {
  ...
  if (!name) return;
  await addDoc(col("categories"), { name, type: "otro", order: state.categories.length + 1 });
  input.value = "";
